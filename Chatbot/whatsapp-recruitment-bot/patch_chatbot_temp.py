...
# For CV, we leave it largely alone because the CV flow uses document handling mostly, 
# although text input for CV should be handled here. The prompt said `STATE_AWAITING_CV` too, wait.
"If they send gibberish or a question during CV upload" -> yes, text fallback for AWAITING_CV exists!
cv_replacement = """        # ── AWAITING CV ───────────────────────────────────────────────────────
        elif state == self.STATE_AWAITING_CV:
            # Revert to universal fallback, because AWAITING_CV should get a file/image, not text
            # Unless it's "no cv" intent.
            if _is_no_cv_intent(text):
                # proceed to manual info extraction
                crud.update_candidate_state(db, candidate.id, self.STATE_COLLECTING_INFO)
                
                data = candidate.extracted_data or {}
                data['cv_status'] = 'no_cv_provided'
                candidate.extracted_data = data
                db.commit()

                return self._get_next_intake_question(candidate, language)
            
            # If not a recognized intent, universal catch-all for text in CV state
            takeover_reply = await rag_engine.generate_global_takeover(
                user_message=text, 
                current_state=candidate.conversation_state
            )
...
# To replace CV, it ends at: '        # ── COLLECTING MISSING INFO'
cv_end = '        # ── COLLECTING MISSING INFO'
content = replace_block(content, cv_start, cv_end, cv_replacement)
