...
> **✅ "NEW UPGRADES" BATCH SHIPPED + DEPLOYED + verified on prod (2026-06-10, commit `7fda5bd`).** Two batches in one deploy. **Batch 1 — conversations-panel upgrades:** candidate-level **counts parity** (shared `services/conversation-counts.js` builder behind `/active-chats(/counts)` + new `GET /api/applications/status-totals`; `X-List-Truncated` + "Candidates by stage" strip on Applications); in-panel **Certify/Interview popups** (`POST /api/candidates/:id/certify` + `/ensure-application`, shared `StageActionDialogs.jsx`, wired into CallRemarksPanel **and** the CV Manager modal — the plain status dropdown stays message-less by design); **Add-candidate + welcome** (`POST /api/candidates/with-welcome` → chatbot `welcome` status seeds intake; `AddCandidateDialog.jsx`); **future-project transfer + inline draft roles** (migration **039** `projects.is_future`; `jobs.status='future'` inline create; `InlineCreateRole.jsx` in transfer + Done→Screening). **Batch 2 — delivery/engagement/bulk fixes:** **agent/takeover messages now route through the chatbot's working token** — new chatbot `POST /webhook/agent-message` + `meta_client.send_media_by_link`; `/api/communications/send` calls `chatbotNotifier.sendAgentMessage` and records honest `delivery_status`; **per-message delivery ticks** (✓/✓✓/blue/⚠ Not sent) in Messages; **engagement action logging** (migration **040** `call_logs.action_type`; certify/interview logged as activity rows; `/api/engagement/call-logs` rollup broken out into calls·messages·screenings·certs·interviews + admin per-agent table); **bulk interviews** throttle + `delivery_summary` per-reason breakdown in the UI. **Prod deploy:** backend `recruitment-backend-00109-99v` (migrations 039+040 verified present on prod, both tables `recruitment_user`-owned); chatbot `whatsapp-chatbot-00141-9qq` + `whatsapp-celery-worker-00106-tf2`; frontend Firebase Hosting (dewan-chatbot-1234.web.app). **Verified:** 8/9 jest suites (only the known chatbot OpenAI-401 fails), vite build, py_compile, + prod smoke (`/api/chatbot/jobs` 200/15 jobs · `/webhook/agent-message` 401-route-live · SPA + `/api` rewrite 200/400/401). **⚠ Outstanding Meta-template activations (user-only, outward-facing — NOT done):** ① approve a **welcome** WhatsApp template + set `TEMPLATE_WELCOME` in chatbot env so the auto-welcome reaches *cold* (never-messaged) numbers — until then agent-added candidates appear in New with "welcome undeliverable (out of 24h window)". ② approve the **interview_scheduled** template + set `TEMPLATE_INTERVIEW_SCHEDULED` so bulk invites reach out-of-window candidates (today those show in the new delivery breakdown as "no reply in 24h"). Agent free-form replies to out-of-window candidates are still Meta-dropped (shown as ⚠ Not sent) — by design. **Follow-up fix (commit `2233dd3`, chatbot rev `whatsapp-chatbot-00142-tqd`):** the bot told a candidate "we don't have Security Officer in Dubai" though two such active roles exist — they were newest-first-sorted to the bottom of a 15-job list and gpt-4o overlooked them. `conversation_agent._active_jobs_summary(focus_text)` now sorts ACTIVE_JOBS **matches-first** (relevance to the candidate's message/role/country) then newest. See [[chatbot-db-schema-selfheal]].
...
> **✅ PHASE 4a SHIPPED + DEPLOYED + verified on prod (2026-06-07, commit `ba842c6`).** Semantic matching + auto-shortlist: OpenAI `text-embedding-3-small` (1536d) over CV + job text, stored as JSONB on `candidates.cv_embedding` / `jobs.embedding` (migration 036, content-hash staleness), cosine in JS (no pgvector). `services/semantic-match.js`; `GET /api/auto-assign/job/:id/shortlist` (ranked + deterministic "why matched"); `POST /api/auto-assign/embed-backfill` (admin/sourcing); `SmartShortlist.jsx` on JobCandidates. Prod: backend `recruitment-backend-00097-rv8` (migration 036); frontend Firebase `8e6ee4c4a5a040b8`. **Embed-backfill run on prod**: 14 jobs + 922 candidates embedded; shortlist verified (e.g. "Security Officer" → top candidates ranked 49/47/46% with why_matched=[security,…]). Embeddings proven (driver↔driver-job 0.68 vs ↔nurse-job 0.24); 0-bug review. **Phase 4b (candidate self-service via bot) deferred** — the riskiest, candidate-facing write-backs; do in a dedicated session.
>
> **✅ Also shipped same deploy (parallel non-phase work, your authorship): bulk candidate import (CSV/XLSX via SheetJS, `/applications/import`, commit `54430e2`) + realtime application sync (useRealtime + emitApplicationChanged, commits `54430e2`/`024dcf8`).** Note [[socketio-single-instance]]: realtime needs Cloud Run pinned to ONE instance.
