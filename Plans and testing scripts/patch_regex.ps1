# patch_regex.ps1 — Replace intent regex blocks in chatbot.py
...
$si_ne    = [char]0x0DB1 + [char]0x0DD1
$si_ne2   = [char]0x0D92 + " " + [char]0x0DB1 + [char]0x0DD1
...
$ta_no    = [char]0x0B87 + [char]0x0BB2 + [char]0x0BCD + [char]0x0BB2 + [char]0x0BC8
$ta_no2   = [char]0x0BB5 + [char]0x0BC7 + [char]0x0BA3 + [char]0x0BCD + [char]0x0B9F + [char]0x0BBE + [char]0x0BAE + [char]0x0BCD
$ta_q     = [char]0x0BAE + [char]0x0BCB + [char]0x0B95 + [char]0x0BA8

# ── Build old block (original text to find) ─────────────────────────────────
...
$oldNo = "_NO_RE = re.compile(" + $nl +
"    r'\b(no|nope|not now|later|maybe later|'" + $nl +
"    r'" + $si_ne + "|" + $si_ne2 + "|'" + $nl +
"    r'" + $ta_no + "|" + $ta_no2 + ")\b'," + $nl +
"    re.IGNORECASE" + $nl +
")"
...
Write-Host "No match:    $($text.Contains($oldNo))"
Write-Host "Q match:     $($text.Contains($oldQ))"

# ── Build new block ──────────────────────────────────────────────────────────
$si_epa   = [char]0x0D91 + [char]0x0DB4 + [char]0x0DCF
...
$newNo = "_NO_RE = re.compile(" + $nl +
"    r'\b(no|nope|not now|later|maybe later|'" + $nl +
"    # Sinhala script" + $nl +
"    r'" + $si_ne + "|" + $si_ne2 + "|" + $si_epa + "|'" + $nl +
"    # Tamil script" + $nl +
"    r'" + $ta_no + "|" + $ta_no2 + "|'" + $nl +
"    # Singlish" + $nl +
"    r'nehe|neha|epa|epaa|naha|behe|'" + $nl +
"    # Tanglish" + $nl +
"    r'illai|illada|illa|ille|venaam|vendaam|venam)\b'," + $nl +
"    re.IGNORECASE" + $nl +
")"
...
if ($text.Contains($oldNo)) {
    $text = $text.Replace($oldNo, $newNo)
    Write-Host "Replaced _NO_RE"
} else {
    Write-Host "WARNING: _NO_RE old block not found"
}
